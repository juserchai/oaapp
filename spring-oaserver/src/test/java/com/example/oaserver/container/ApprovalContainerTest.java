...
public class ApprovalContainerTest extends AbstractContainerBaseTest {
    ...
    void testApprovalLifecycle() throws Exception {
        ...
        String approvalId = root.path("id").asText();
        ...
        assertEquals(ApprovalStatus.PENDING.name(), getRoot.path("status").asText());
        ...
        assertEquals(ApprovalStatus.APPROVED.name(), finalRoot.path("status").asText());
    }
    ...
    void testFilterApprovalsByType() throws Exception {
        ...
        boolean foundLeaveApproval = false;
        for (JsonNode node : root) {
            if (node.get("title").asText().equals("请假申请") && 
                node.get("type").asText().equals(ApprovalType.LEAVE.name())) {
                foundLeaveApproval = true;
                break;
            }
        }
        assertTrue(foundLeaveApproval, "Should find the leave approval request");
    }
